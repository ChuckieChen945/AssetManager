...
def buildGeo(
    ...
    if assetType == "3D":
        ...
        geoPath, proxyPath = getGeoAndProxyPath(
            assetObjFolderList, geoFormat, lod, lodProxy, assetObjFolder
        )
        ...
        for i in range(numVars):
            ...
            if lodProxy != "None":
                ...
                createGeoVar(
                    node, proxyVarCreate, assetName, proxyPath, i, path, assetType, geoFormat
                )
    ...
    # Process plant type asset
        ...
        for i in range(numVars):
            ...
            # Get geo and proxy files
            geoPath, proxyPath = getGeoAndProxyPath(
                varList, geoFormat, lod, lodProxy, varAssetFolder
            )
...
def createBuildDialog():
    ...
    if os.path.isdir(assetDir) == False:
        hou.ui.displayMessage(
            "Error: Asset folder does not exists.", severity=hou.severityType.Error
        )
        return

    if len(assetName) <= 1:
        hou.ui.displayMessage(
            "Error: Asset Name should contain 2 or more letters.", severity=hou.severityType.Error
        )
        return
    ...
    if len(LOD) == 0 or len(geoFormat) == 0:
        hou.ui.displayMessage(
            "Error: Can't find any abc, fbx or obj geometry.", severity=hou.severityType.Error
        )
        return
